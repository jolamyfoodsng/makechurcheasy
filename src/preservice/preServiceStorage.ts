...
export function loadPreServicePlan(): PreServicePlan {
  try {
    const raw = localStorage.getItem(getUserScopedKey(PLAN_KEY));
...
export function savePreServicePlan(plan: PreServicePlan): void {
  try {
    localStorage.setItem(getUserScopedKey(PLAN_KEY), JSON.stringify(plan));
  } catch {
...
export function loadPreServiceRuntime(): PreServiceRuntimeState {
  try {
    const raw = localStorage.getItem(getUserScopedKey(RUNTIME_KEY));
...
export function savePreServiceRuntime(runtime: PreServiceRuntimeState): void {
  try {
    localStorage.setItem(getUserScopedKey(RUNTIME_KEY), JSON.stringify(runtime));
  } catch {
    // ignore storage failures
  }
}
