...
/* ═══════════════════════════════════════════════════════════════════
   ...
   called MCE_PreService.  The engine creates/updates each source,
...
const PRESERVICE_SCENE = "MCE_PreService";
...
/** Enable or disable a scene item in MCE_PreService */
...
function sourceNameForStep(step: PreServiceStep, index: number): string {
  ...
  return `MCE_PS_${String(index).padStart(2, "0")}_${safe}`;
}
...
/**
 * Set up all steps as sources in MCE_PreService.
 ...
 * Switches to MCE_PreService on the first call.
 */
export async function activateStep(
  ...
  // Switch to MCE_PreService on the first activation
  if (!sceneOnProgram) {
