  ...
  try {
    ...
    for (const region of sorted) {
      if (region.type === "obs-scene") {
        ...
        if (isThemedSlot || isLTSlot) {
          ...
          desiredSources.add(`MCE MV: ${srcName}`);
    ...
    for (const region of sorted) {
      try {
        ...
        switch (region.type) {
          ...
          case "obs-scene": {
            ...
            if (isThemedSlot && region.themeSettings) {
              ...
              const overlaySceneName = `MCE MV: ${srcName}`;
            ...
            if (isLTSlot) {
              ...
              // Create/update the overlay scene and browser source inside it
              const ltOverlaySceneName = `MCE MV: ${srcName}`;
        ...
        // 4a. Play intro animation if configured (non-blocking)
        if (region.animation?.intro && region.animation.intro !== "none") {
          const animDeps = {
            transformSceneItem: async (sn: string, siid: number, x: number, y: number, w: number, h: number, opts?: { opacity?: number; cropLeft?: number; cropRight?: number; cropTop?: number; cropBottom?: number }) => {
              ...
              // Apply opacity via existing filter if needed
              if (opts?.opacity !== undefined && opts.opacity < 1) {
                ...
                try {
                  await obsService.call("SetSourceFilterSettings", {
                    sourceName: srcName,
                    filterName,
                    filterSettings: { opacity: opts.opacity },
                  });
                } catch { /* filter may not exist yet */ }
              }
            },
            setSceneItemEnabled: async (sn: string, siid: number, enabled: boolean) => {
              await obsService.call("SetSceneItemEnabled", { sceneName: sn, sceneItemId: siid, sceneItemEnabled: enabled });
            },
            sleep: (ms: number) => new Promise<void>((r) => setTimeout(r, ms)),
            getCanvasSize: async () => layout.canvas,
          };
          // Fire and forget — don't block the rest of the push
          void playIntroAnimation(
            ...
            console.warn(`[MV→OBS] Intro animation failed for "${region.name}":`, err);
          });
        }
