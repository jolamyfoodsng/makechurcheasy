...
import { regionTypeLabel, regionTypeIcon, type Region, type RegionConstraints, type RegionAnimation, DEFAULT_REGION_ANIMATION, INTRO_ANIMATION_OPTIONS, OUTRO_ANIMATION_OPTIONS, ANIMATION_EASING_OPTIONS } from "../types";
  ...
  return (
    <div className="mv-inspector" role="complementary" aria-label="Slot inspector">
      ...
      <AnimationPanel
        ...
        onChange={(anim) => update({ animation: anim })}
      />
    </div>
  );
}
...
// ═══════════════════════════════════════════════════════════════════════════
// AnimationPanel — per-region intro/outro animation controls
// ═══════════════════════════════════════════════════════════════════════════
...
function AnimationPanel({
  ...
  onChange: (anim: RegionAnimation) => void;
}) {
  const anim = animation ?? DEFAULT_REGION_ANIMATION;

  const update = (partial: Partial<RegionAnimation>) => {
    onChange({ ...anim, ...partial });
  };

  return (
    <div className="mv-inspector-section">
      <h4 className="mv-inspector-section-title">Animation</h4>

      {/* Intro Effect */}
      <div className="mv-field">
        <label className="mv-field-label">Intro Effect</label>
        <select
          className="mv-field-input"
          value={anim.intro}
          onChange={(e) => update({ intro: e.target.value as RegionAnimation["intro"] })}
        >
          {INTRO_ANIMATION_OPTIONS.map((opt) => (
            <option key={opt.value} value={opt.value}>{opt.label}</option>
          ))}
        </select>
      </div>
      ...
      {/* Outro Effect */}
      <div className="mv-field">
        <label className="mv-field-label">Outro Effect</label>
        <select
          className="mv-field-input"
          value={anim.outro}
          onChange={(e) => update({ outro: e.target.value as RegionAnimation["outro"] })}
        >
          {OUTRO_ANIMATION_OPTIONS.map((opt) => (
            <option key={opt.value} value={opt.value}>{opt.label}</option>
          ))}
        </select>
      </div>
      ...
      <div className="mv-field-grid">
        <div className="mv-field">
          <label className="mv-field-label">Duration</label>
          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
            ...
            />
            <span style={{ fontSize: 10, color: "var(--dock-text-dim)", flexShrink: 0 }}>ms</span>
          </div>
        </div>

        <div className="mv-field">
          <label className="mv-field-label">Easing</label>
          <select
            className="mv-field-input"
            value={anim.easing}
            onChange={(e) => update({ easing: e.target.value as RegionAnimation["easing"] })}
          >
