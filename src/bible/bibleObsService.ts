/**
 * bibleObsService.ts — Broadcast integration for Bible overlays
 *
 ...
 * Bible source lives inside MCE Presentation scene alongside other module sources.
 */
...
class BibleObsService {
  ...
  private async _ensureBrowserSourceImpl(
    ...
    // 2. The "MCE Bible" source already exists inside the presentation scene
    // 3. We just need to update the source URL and show it
    // ═══════════════════════════════════════════════════════════════════════

    const overlaySceneName = BIBLE_SCENE_NAME; // "MCE Presentation"
    ...
    let currentSourceName: string = BIBLE_SOURCE_NAME;
    ...
    if (browserItemId === null) {
      // Create the browser source inside the presentation scene
      try {
        ...
        if (msg.includes("already exists") || msg.includes("600")) {
          ...
          // Add to presentation scene if not already there
          try {
    ...
    // ── 3b. Duplicate BG source — flicker safety net ──
   ...
   * color and never replaced. It sits at z-index 0 so when the browser
  ...
  private async ensureDupBgSource(sceneName: string): Promise<void> {
    ...
    if (this._dupBgSceneItemId !== null) {
      try {
        const resp = await obsService.call("GetSceneItemList", { sceneName });
        const items = (resp as { sceneItems: Array<{ sourceName: string; sceneItemId: number }> }).sceneItems ?? [];
        const exists = items.find((item) => item.sourceName === BIBLE_DUP_BG_SOURCE_NAME);
        if (exists) {
          this._dupBgSceneItemId = exists.sceneItemId;
          return;
        }
        // Source was removed from scene — re-add it
        const bgItemId = await obsService.createSceneItem(sceneName, BIBLE_DUP_BG_SOURCE_NAME);
    ...
    try {
      const inputs = await obsService.getInputList();
      inputExists = inputs.some((i) => i.inputName === BIBLE_DUP_BG_SOURCE_NAME);
    } catch { /* ok */ }
    ...
    try {
      ...
      // Always at z-index 0 — behind everything
      await obsService.call("SetSceneItemIndex", {
        sceneName,
      ...
      await this.enableSceneItemSafe(sceneName, this._dupBgSceneItemId!);
    } catch (err) {
      console.warn("[BibleOBS] Failed to create dup BG safety source:", err);
    }
  }
  ...
  private async updateDupBgColor(_sceneName: string, theme: BibleThemeSettings): Promise<void> {
    ...
    const fingerprint = `dup:${bgColor.toLowerCase()}:${bgOpacity}:${hasGradient ? "gradient" : "solid"}`;

    if (fingerprint === this._dupBgFingerprint) return;

    try {
      // When gradient mode is active, the dup BG source must be transparent
      // since the browser source renders the gradient via CSS.
      const obsColor = hasGradient
        ? 0x00000000
        : this.hexToObsColor(bgColor, bgOpacity);
      await obsService.call("SetInputSettings", {
        inputName: BIBLE_DUP_BG_SOURCE_NAME,
        inputSettings: { color: obsColor },
      });
      this._dupBgFingerprint = fingerprint;
    } catch (err) {
      console.warn("[BibleOBS] Failed to update dup BG color:", err);
    }
  }
  ...
  async pushSlide(
    ...
    if (obsService.isConnected) {
      ...
      try {
        ...
        let resolvedInputName: string = BIBLE_SOURCE_NAME;
        const regInput = await getInputBySlot(SLOT_INPUT);
        if (regInput) {
          const inputs = await obsService.getInputList();
          const found = inputs.find((i) => i.inputUuid === regInput.inputUuid);
            ...
            if (bgFingerprint !== this._lastBgFingerprint) {
              ...
              if (hasImage) {
                ...
                if (this._currentBgKind !== "image") {
                  ...
                  let resolvedBgName: string = BIBLE_BG_SOURCE_NAME;
                  const regBg = await getInputBySlot(SLOT_BG_INPUT);
                  if (regBg) {
                    const inputs = await obsService.getInputList();
  ...
  async clearOverlay(sceneNames?: string[]): Promise<void> {
    ...
    // Disable dup BG safety source so it doesn't show as a solid rectangle
    if (this._dupBgSceneItemId !== null) {
      try {
        await obsService.call("SetSceneItemEnabled", {
  ...
  /**
   * Toggle blank state for the current live slide.
   * Returns the new blanked state.
   */
  async toggleBlank(): Promise<boolean> {
    this._isBlanked = !this._isBlanked;
    if (this._liveSlide) {
      await this.pushSlide(
        this._liveSlide,
        this._liveTheme,
        true,
        this._isBlanked,
        this._liveTemplateType
      );
    }
    return this._isBlanked;
  }
  ...
  // Uses FullscreenSceneManager — the scene IS the output, not an overlay.
  // ═══════════════════════════════════════════════════════════════════════

  /**
   * Show Bible content as a fullscreen scene (not an overlay).
   * Creates the MCE scene if missing, updates browser source content,
   * and pushes the scene to OBS Preview/Program.
   */
  async showFullscreen(
    ...
    this._isLive = live;
    this._isBlanked = blanked;
    this._liveTemplateType = "fullscreen";

    overlayBroadcaster.pushSlide(slide, theme, live, blanked);

    if (!obsService.isConnected) return;

    const def = FULLSCREEN_SCENES.bible;

    // Ensure scene exists
    await fullscreenSceneManager.ensureScene(def);

    // Build packet for browser source
    const { themeForHash, customCss } = this.buildThemePayload(theme);
    const packet = {
      slide,
      theme: themeForHash,
  ...
  /**
   * Hide the Bible fullscreen scene by transitioning away from it.
   * Does NOT destroy the scene.
   */
  ...
  /**
   * Check if the Bible fullscreen scene is currently active.
   */
  async isFullscreenActive(): Promise<boolean> {
    if (!obsService.isConnected) return false;
    return fullscreenSceneManager.isActive(FULLSCREEN_SCENES.bible);
  }

  /**
   * Update the Bible fullscreen scene content without switching scenes.
   * Used for verse-by-verse navigation while already live.
   */
