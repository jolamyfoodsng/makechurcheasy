 ...
 * The Fly.io server checks this and rejects versions below its minimum floor.
 * If the header is missing (4.28 and earlier), the server should also reject.
 */
 ...
 * New clients call this to extract the actual payload.
 */
 ...
 * Results are cached in memory for the session so repeated default
 * queries (e.g. opening the library modal) don't hit the network.
 */
const _catalogCache = new Map<string, CatalogResponse>();

/** Return a cached catalog result for the given params, or null. */
export function getCachedCatalogResult(params: CatalogSearchParams = {}): CatalogResponse | null {
  return _catalogCache.get(catalogCacheKey(params)) ?? null;
}

function catalogCacheKey(params: CatalogSearchParams): string {
  return [
    params.query ?? "",
    params.language ?? "",
    params.country ?? "",
...
export async function searchCatalog(
  ...
  try {
    ...
    if (Array.isArray(json)) {
      ...
      result = { items, total, page, limit, pages };
    }
    _catalogCache.set(key, result);
    return result;
  } catch (err) {
...
export async function getCatalogBible(id: string): Promise<CatalogBible> {
  ...
  const raw = await res.json();
  return unwrapEnvelope<CatalogBible>(raw);
}
...
async function getDownloadUrl(id: string): Promise<string> {
  ...
  const raw = await res.json();
  const data = unwrapEnvelope<{ url: string }>(raw);
  return data.url;
}
