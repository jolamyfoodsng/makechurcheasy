...
class VoiceBibleService {
  ...
  private audioHealthTimer: ReturnType<typeof setInterval> | null = null;
  ...
  private applyLiveMatchIntent(
    ...
    if (intent.kind === "result") {
      ...
      this.pushCandidates();
      this.pushResult();
      track("voice_semantic_match_found", {
        confidence: (intent.result as unknown as Record<string, unknown>).confidence ?? 0,
      });
      if (intent.result.action === "stage-verse") {
        track("voice_reference_detected", {
          directReference: `${intent.result.book ?? ""} ${intent.result.chapter ?? ""}:${intent.result.verse ?? ""}`.trim(),
        });
      }
      return;
    }
  ...
  private async startListening(context: VoiceBibleContextPayload | null): Promise<void> {
    ...
    try {
      ...
      // ── Background resilience: recover AudioContext and log focus/visibility ──

      this.visibilityHandler = () => {
        const hidden = document.hidden;
        if (!hidden && audioContext.state === "suspended") {
      ...
      // Periodic audio pipeline health check
      this.lastAudioChunkTime = Date.now();
      this.audioHealthTimer = setInterval(() => {
        const elapsed = Date.now() - this.lastAudioChunkTime;
        if (elapsed > 5000 && this.snapshot.status === "listening") {
          console.warn(
            "[Audio] No audio chunks for", elapsed, "ms — pipeline may be stalled.",
  ...
  private async teardownAudio(): Promise<void> {
    // Remove focus/visibility handlers and health timer
    ...
    if (this.audioHealthTimer) {
      clearInterval(this.audioHealthTimer);
