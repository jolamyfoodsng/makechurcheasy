...
class OBSService {
  ...
  private static RECONNECT_MAX_DELAY_MS = 30000;
  private static MAX_RECONNECT_ATTEMPTS = 50;

  constructor() {
    this.obs = new OBSWebSocket();

    // When OBS closes the connection (e.g. brief network hiccup or OBS restart),
    // debounce before declaring disconnected — gives time for reconnect to succeed.
    this.obs.on("ConnectionClosed", () => {
      // Don't reset connectLock here — the reconnect flow handles it
  ...
  private scheduleReconnect(): void {
    if (!this.autoReconnect) {
      this.declareDisconnected();
      return;
    }
    if (this.reconnectAttempts >= OBSService.MAX_RECONNECT_ATTEMPTS) {
      console.warn("[OBSService] Max reconnect attempts reached — giving up");
      this.declareDisconnected("Lost connection to OBS — click Connect to retry");
      return;
    }

    // Clear any pending disconnect broadcast — we're going to try reconnecting
    ...
    const delay = Math.min(
      OBSService.RECONNECT_BASE_DELAY_MS * Math.pow(1.5, this.reconnectAttempts),
      OBSService.RECONNECT_MAX_DELAY_MS
    );
    this.reconnectAttempts++;

    console.log(`[OBSService] Reconnecting in ${Math.round(delay)}ms (attempt #${this.reconnectAttempts})`);
    ...
    this.reconnectTimer = setTimeout(async () => {
      ...
      try {
        ...
        await this.obs.connect(this.lastUrl, this.lastPassword, { rpcVersion: 1 });
        // Success!
        ...
        // Failed — will trigger another ConnectionClosed/Error → scheduleReconnect cycle
        this.connectLock = false;
      }
    }, delay);
  }

  /** Broadcast "disconnected" — only after grace period expires or explicitly */
  private declareDisconnected(error?: string): void {
    if (this.disconnectDebounceTimer) {
      clearTimeout(this.disconnectDebounceTimer);
      this.disconnectDebounceTimer = null;
    }
  ...
  setAutoReconnect(enabled: boolean): void {
    ...
    if (!enabled) {
      ...
      if (this.disconnectDebounceTimer) { clearTimeout(this.disconnectDebounceTimer); this.disconnectDebounceTimer = null; }
      this.reconnectAttempts = 0;
    }
  }
  ...
  async connect(
    ...
    // Clear any pending reconnect — we're taking over with an explicit connect
    if (this.reconnectTimer) { clearTimeout(this.reconnectTimer); this.reconnectTimer = null; }
    if (this.disconnectDebounceTimer) { clearTimeout(this.disconnectDebounceTimer); this.disconnectDebounceTimer = null; }
    this.reconnectAttempts = 0;
  ...
  async createSceneItem(
    ...
    if (this._status !== "connected") {
      console.warn(`[OBSService] Not connected, cannot create scene item "${sourceName}" in "${sceneName}"`);
      return 0;
    }
    if (!sceneName || !sceneName.trim()) {
      console.warn(`[OBSService] Invalid sceneName, cannot create scene item for "${sourceName}"`);
      return 0;
    }
    if (!sourceName || !sourceName.trim()) {
      console.warn(`[OBSService] Invalid sourceName, cannot create scene item in "${sceneName}"`);
      return 0;
    }

    try {
      const existing = await this.obs.call("GetSceneItemId", {
        sceneName,
        sourceName,
      });
      const id = existing.sceneItemId as number;
      return id;
    } catch {
      // Not found in scene, continue to create
    }
    ...
    try {
      ...
      console.warn(
        `[OBSService] Failed to create scene item "${sourceName}" in "${sceneName}":`, err
      );
      return 0;
    }
  }
