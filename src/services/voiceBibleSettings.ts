...
function getScopedSettingsKey(): string {
  const uid = getCurrentUserId();
  return uid ? `${VOICE_BIBLE_SETTINGS_KEY}:${uid}` : VOICE_BIBLE_SETTINGS_KEY;
}
...
export async function getVoiceBibleSettings(): Promise<VoiceBibleSettings> {
  ...
  if (scopedKey !== VOICE_BIBLE_SETTINGS_KEY) {
    const legacy = await getByKey<VoiceBibleSettings>(
      STORES.APP_SETTINGS,
      VOICE_BIBLE_SETTINGS_KEY,
    ).catch(() => undefined);
    if (legacy) {
      await putRecord(STORES.APP_SETTINGS, legacy, scopedKey);
      return normalizeVoiceBibleSettings(legacy);
    }
  }
