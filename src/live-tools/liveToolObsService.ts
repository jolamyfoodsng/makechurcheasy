...
const LIVE_TOOL_SOURCE = "MCE Live Tools";
const PREVIEW_LIVE_TOOL_SOURCE = "MCE Preview Live Tools";
const LIVE_TOOL_MEDIA_VIDEO_SOURCE = "MCE Live Tools Media Video";
const LIVE_TOOL_MEDIA_IMAGE_SOURCE = "MCE Live Tools Media Image";
const PREVIEW_LIVE_TOOL_MEDIA_VIDEO_SOURCE = "MCE Preview Live Tools Media Video";
const PREVIEW_LIVE_TOOL_MEDIA_IMAGE_SOURCE = "MCE Preview Live Tools Media Image";
...
async function hideManagedSources(sceneName: string): Promise<void> {
  ...
  await Promise.all(items.map(async (item) => {
    if (!item.sourceName.includes("OCS") && !item.sourceName.includes("MCE ")) return;
    try {
      await obsService.call("SetSceneItemEnabled", {
        sceneName,
        sceneItemId: item.sceneItemId,
