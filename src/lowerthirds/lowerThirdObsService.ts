/**
 * lowerThirdObsService.ts — Broadcast integration for Lower Third overlays
 *
 * Manages browser sources in OBS that display lower-third themes.
 * Supports:
 *   - Pushing theme + variable values to MakeChurchEasy-managed LT sources
 *   - Discovering all LT sources in OBS (MakeChurchEasy-managed + MV-created)
...
class LowerThirdObsService {
  ...
  private isLowerThirdSourceName(sourceName: string): boolean {
    return OCS_LT_PATTERN.test(sourceName) || VC_LT_PATTERN.test(sourceName) || MV_LT_PATTERN.test(sourceName) || OCS_BIBLE_LT_PATTERN.test(sourceName) || VC_BIBLE_LT_PATTERN.test(sourceName) || sourceName.startsWith("MCE LT:");
  }
  ...
  /**
   * Ensure a dedicated MakeChurchEasy LT scene exists with a browser source inside.
   * The scene is named "MCE LT: <sourceName>" and contains one browser_source.
  ...
  async ensureSource(sourceName?: string): Promise<{ sceneName: string; sourceName: string }> {
    ...
    const ltOverlayScene = `MCE LT: ${name}`;
   ...
   * Returns both MakeChurchEasy-managed (VC_LT_*) and MV-created (MV_*_LT:*) sources.
   */
  async discoverSources(): Promise<LTObsSource[]> {
    ...
    try {
      ...
      for (const input of inputs) {
        ...
        const isOcs = OCS_LT_PATTERN.test(input.inputName) || VC_LT_PATTERN.test(input.inputName);
        const isMv = MV_LT_PATTERN.test(input.inputName);
        const isBibleLt = OCS_BIBLE_LT_PATTERN.test(input.inputName) || VC_BIBLE_LT_PATTERN.test(input.inputName);
   ...
   * Creates a dedicated "MCE LT: <sourceName>" scene with a browser source inside,
  ...
  async ensureSourceInScene(targetScene: string): Promise<string> {
    ...
    const ltOverlayScene = `MCE LT: ${sourceName}`;
  ...
  async hideExistingLTSourcesInScene(targetScene: string, keepSourceName: string): Promise<void> {
    if (!obsService.isConnected) return;
    const keepOverlayScene = `MCE LT: ${keepSourceName}`;
    try {
      const resp = await obsService.call("GetSceneItemList", { sceneName: targetScene }) as {
      ...
      for (const item of resp.sceneItems) {
        ...
        // Hide any MCE LT / OCS LT overlay scenes, MCE_LT_, MCE_BibleLT_, or MV_*_LT: sources
        const isOcsLtOverlay = item.sourceName.startsWith("MCE LT:") || item.sourceName.startsWith("OCS LT:");
        if (isOcsLtOverlay || OCS_LT_PATTERN.test(item.sourceName) || VC_LT_PATTERN.test(item.sourceName) || MV_LT_PATTERN.test(item.sourceName) || OCS_BIBLE_LT_PATTERN.test(item.sourceName) || VC_BIBLE_LT_PATTERN.test(item.sourceName)) {
          try {
  ...
  async pushToAll(
    ...
    // If no LT sources found, log a warning instead of auto-creating VC_LT_Main.
