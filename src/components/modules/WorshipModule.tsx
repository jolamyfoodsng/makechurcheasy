...
interface WorshipLayoutPrefs {
  layoutMode: BibleTemplateType;
  activeThemeId: string;
  activeWorshipLowerThirdId: string;
  primaryColor: string;
}

function loadWorshipLayoutPrefs(): WorshipLayoutPrefs | null {
  try {
    const raw = localStorage.getItem(getUserScopedKey(WORSHIP_LAYOUT_PREFS_KEY));
    return raw ? JSON.parse(raw) : null;
  } catch { return null; }
}

function saveWorshipLayoutPrefs(prefs: WorshipLayoutPrefs) {
  try {
...
export function WorshipModule({
  ...
  // ── Theme state (persisted across navigation) ──
  const savedWorshipPrefs = loadWorshipLayoutPrefs();
  const [themes, setThemes] = useState<BibleTheme[]>(WORSHIP_FULLSCREEN_THEME_FALLBACKS);
  const [layoutMode, setLayoutMode] = useState<BibleTemplateType>(savedWorshipPrefs?.layoutMode ?? "fullscreen");
  const [activeThemeId, setActiveThemeId] = useState(savedWorshipPrefs?.activeThemeId ?? (WORSHIP_FULLSCREEN_THEME_FALLBACKS[0]?.id ?? "classic-dark"));
  const [themeOpen, setThemeOpen] = useState(false);
  const [activeWorshipLowerThirdId, setActiveWorshipLowerThirdId] = useState(savedWorshipPrefs?.activeWorshipLowerThirdId ?? DEFAULT_WORSHIP_THEME_ID);
  const [worshipThemeOpen, setWorshipThemeOpen] = useState(false);
  const [primaryColor, setPrimaryColor] = useState(savedWorshipPrefs?.primaryColor ?? "#4ADE80");
  ...
  // Persist worship layout preferences across navigation
  useEffect(() => {
    saveWorshipLayoutPrefs({ layoutMode, activeThemeId, activeWorshipLowerThirdId, primaryColor });
  }, [layoutMode, activeThemeId, activeWorshipLowerThirdId, primaryColor]);
  ...
  const importTitleWasAutoGeneratedRef = useRef(false);
  ...
  // Recompute song count whenever song list changes
  useEffect(() => {
    if (songsLoaded) computeSongCount();
  }, [songs.length, songsLoaded, computeSongCount]);

  // Load fullscreen Bible/Worship Broadcast themes (built-in + custom from theme DB)
  ...
  const pushCurrentWorshipViaDock = useCallback(async (): Promise<void> => {
    const slide = songSlides[liveSlideIndex];
  ...
  const handleSendWorshipFullToScene = useCallback(async (
    ...
    if (dockLive !== null) {
      ...
      try {
        setIsLive(true);
        setIsBlanked(false);
        await pushCurrentWorshipViaDock();
        setFullLiveScenes((prev) => (prev.includes(sceneName) ? prev : [...prev, sceneName]));
        if (!wasLive && serviceStore.status === "preservice") {
  ...
  const handleSaveImport = useCallback(async () => {
    let finalTitle = importMetadata.title.trim();
    if (importTitleWasAutoGeneratedRef.current || isDummyTitle(finalTitle) || !finalTitle) {
      const derived = extractFirstLineAsTitle(importLyrics);
    ...
    const newSong: Song = {
      id: `song-${Date.now()}`,
      metadata: { ...importMetadata, title: finalTitle },
      lyrics: importLyrics,
    ...
    importTitleWasAutoGeneratedRef.current = false;
    track("song_created", { autoSplit: false });
    track("song_imported", { source: "paste" });
  }, [hasReachedSongLimit, importMetadata, importLyrics, reloadSongs, splitConfig]);

  const handleImportTitleChange = useCallback((title: string) => {
    importTitleWasAutoGeneratedRef.current = isDummyTitle(title);
    setImportMetadata((metadata) => ({ ...metadata, title }));
  }, []);

  const handleImportLyricsChange = useCallback((nextLyrics: string) => {
    setImportLyrics(nextLyrics);
    if (!importTitleWasAutoGeneratedRef.current && !isDummyTitle(importMetadata.title) && importMetadata.title.trim()) return;

    const derivedTitle = extractFirstLineAsTitle(nextLyrics);
    if (derivedTitle) {
      importTitleWasAutoGeneratedRef.current = true;
      setImportMetadata((metadata) => ({ ...metadata, title: derivedTitle }));
    }
          ...
          <div className="worship-import-meta">
            <div className="worship-import-field">
              <label>
                ...
                <span className="worship-import-field__tag worship-import-field__tag--required">Required</span>
              </label>
              <input
                type="text"
                placeholder="Enter song title"
                value={importMetadata.title}
                onChange={(e) => handleImportTitleChange(e.target.value)}
              />
            </div>
            <div className="worship-import-field">
              <label>
                <span>Artist</span>
                <span className="worship-import-field__tag">Optional</span>
              </label>
              <input
                type="text"
        ...
        <div className="worship-import-body">
          <section className="worship-lyrics-editor">
            ...
            <textarea
              ...
              value={importLyrics}
              onChange={(e) => handleImportLyricsChange(e.target.value)}
            />
          </section>
      ...
      <div className={`worship-home${presentationMode ? " worship-home--presentation" : ""}`}>
        ...
        <div className={`worship-main${showLiveSlideStrip ? " has-live-strip" : ""}`}>
          ...
          <aside className="worship-sidebar">
            ...
            {!isSongUnlimited && songsLoaded && (
              <div className={`worship-sidebar-usage${hasReachedSongLimit ? " at-limit" : ""}`}>
                <span className="worship-sidebar-usage-text">
