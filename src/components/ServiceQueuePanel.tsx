...
export function ServiceQueuePanel({ onRefreshLibrary }: Props) {
    ...
    const handleFadeToBlack = useCallback(async () => {
        ...
        if (blankPreset) {
            await setActiveContent(blankPreset);
        }
    }, [setActiveContent]);
    ...
    const handleSafeFallback = useCallback(async () => {
        ...
        if (fullPastor) {
            await setActiveContent(fullPastor);
        }
    }, [setActiveContent]);
                ...
                {state.queue.length === 0 ? (
                    ...
                    state.queue.map((qItem, idx) => (
                        <QueueRow
                            ...
                            item={qItem}
                            index={idx}
                            isActive={state.queueIndex === idx && state.activeContent?.id === qItem.content.id}
                            isNext={idx === state.queueIndex + 1}
                            onGoTo={() => handleGoTo(qItem.queueId)}
                            onRemove={() => handleRemove(qItem.queueId)}
                            onMoveUp={() => handleMoveUp(idx)}
                            onMoveDown={() => handleMoveDown(idx)}
                            onSetActive={() => setActiveContent(qItem.content)}
                            isFirst={idx === 0}
...
function QueueRow({
    ...
    return (
        <div
            className={`queue-item ${isActive ? "queue-item-active" : ""} ${isNext ? "queue-item-next" : ""} ${item.played ? "queue-item-played" : ""}`}
        ...
        >
            ...
            <div className="queue-item-info">
                ...
                {isActive && <span className="queue-badge queue-badge-active">ACTIVE</span>}
                {isNext && !isActive && <span className="queue-badge queue-badge-next">NEXT</span>}
            </div>
