...
type StepTypeCardId = "video" | "image" | "countdown" | "audio" | "scene" | "template";
...
function loadAudioLibrary(): AudioLibraryItem[] {
  try {
    const raw = localStorage.getItem(getUserScopedKey(AUDIO_LIBRARY_STORAGE_KEY));
    if (!raw) return [];
...
function saveAudioLibrary(items: AudioLibraryItem[]): void {
  try {
    localStorage.setItem(getUserScopedKey(AUDIO_LIBRARY_STORAGE_KEY), JSON.stringify(items));
  } catch {
...
export default function PreServicePanel() {
  ...
  useEffect(() => {
    ...
    const pollId = window.setInterval(checkStreamStatus, getRecommendedPollingInterval(5000));
  ...
  const executeStep = useCallback(async (
    ...
    try {
      await runPreServiceStep(step, durationSeconds);
    } catch (err) {
      if (token !== runTokenRef.current) return;
  ...
  }, [clearTicker, getStepDuration, holdStepForManualAdvance, setRuntimeSafe]);
    ...
    <div className="pre-service-auto">
      ...
      <div className="ps-page-header">
        <div>
          <h2>{headerTitle}</h2>
          <p>
            Control what appears on screen before service starts. Each step has a duration (timer icon)
      ...
      <div className="ps-grid">
        ...
        <div className="ps-steps-col">
          {plan.steps.map((step, index) => {
            ...
            const autoEnabled = step.autoAdvance !== false;
                ...
                <div className="ps-step-header" onClick={() => toggleStepExpanded(step.id)}>
                  ...
                  <div className="ps-step-header-right">
                    ...
                    <label
                      ...
                      title="When off, this step waits for Skip/Resume after its timer ends."
