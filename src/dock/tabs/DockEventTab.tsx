/**
 * DockEventTab.tsx — Event tab for the MakeChurchEasy Dock
 *
...
export default function DockEventTab({ staged, onStage }: Props) {
  ...
  const handleAdd = useCallback(async () => {
    ...
    if (!(await requireEntitlement("lowerThirds", events.length))) return;
    const ev: DockEvent = {
      id: `evt-${nextEventId++}`,
      name: newName.trim(),
      date: newDate.trim(),
    ...
    setNewLocation("");
    setNewDescription("");
  }, [events, newName, newDate, newLocation, newDescription]);
  ...
  const pushEvent = useCallback(async (
    ...
    onStage({
      ...
      data: { ...ev, ltTheme: themeOverride ?? selectedTheme },
    });
    ...
    try {
      ...
      console.warn("[DockEventTab] Push event failed:", err);
    }
  }, [events, onStage, selectedTheme]);
  ...
  const handleSelectTheme = useCallback(
    (theme: DockLTThemeRef) => {
      ...
      if (selectedId) {
        ...
        if (ev) {
          void pushEvent(ev, theme);
        }
      }
    },
    [events, pushEvent, selectedId]
  );
