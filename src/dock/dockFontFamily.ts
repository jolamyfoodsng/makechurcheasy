...
export function buildDockFontFamilyCss(value: unknown): string {
  ...
  return [
    `.dock-root { --font-family: ${safeFamily}; --ref-font-family: ${safeFamily}; --compare-font-family: ${safeFamily}; --mce-output-font-family: ${safeFamily}; }`,
    `.dock-root, .dock-root *:not(.material-icons):not(.material-icons-outlined):not(.material-icons-round):not(.material-icons-sharp):not(.material-symbols-outlined):not(.fa):not([class^="fa-"]):not([class*=" fa-"]) { font-family: ${safeFamily} !important; }`,
  ].join("\n");
}
