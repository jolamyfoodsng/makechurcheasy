...
export default function ServicePlannerPage() {
  ...
  useEffect(() => {
    if (requestedPlanId) {
      setActivePlanId(requestedPlanId);
    }
  }, [requestedPlanId]);

  const focusedCueId = requestedCueId || activePlan?.selectedItemId || "";

  useEffect(() => {
    if (!focusedCueId) return;
    const timer = window.setTimeout(() => {
      const element = document.querySelector<HTMLElement>(`[data-cue-id="${focusedCueId.replace(/"/g, '\\"')}"]`);
      element?.scrollIntoView({ block: "center", behavior: "smooth" });
    }, 0);
    return () => window.clearTimeout(timer);
  }, [focusedCueId, activePlanId]);
