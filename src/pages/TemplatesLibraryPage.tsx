 ...
 *   - Favorites appear first in the MakeChurchEasy Dock theme pickers
...
const FALLBACK_LOGO_DATA_URI =
  ...
  encodeURIComponent(
    `<svg xmlns="http://www.w3.org/2000/svg" width="220" height="220" viewBox="0 0 220 220">
      <rect width="220" height="220" rx="36" fill="#6A34DE"/>
      <path d="M110 42l46 26v52c0 33-20 56-46 64-26-8-46-31-46-64V68l46-26z" fill="#fff"/>
      <path d="M110 70c17 0 30 13 30 30s-13 30-30 30-30-13-30-30 13-30 30-30z" fill="#6A34DE"/>
    </svg>`
  );
...
function themeTimestamp(theme: UnifiedTheme): number {
  return new Date(theme.updatedAt || theme.createdAt || 0).getTime();
}
...
export default function TemplatesLibraryPage() {
  ...
  const filteredThemes = useMemo(() => {
    ...
    // Sort: favorites first, then newest custom themes, then alphabetical
    return list.sort((a, b) => {
      ...
      if (a.source === "custom" && b.source === "custom") {
        return themeTimestamp(b) - themeTimestamp(a);
      }
            ...
            <div className="tlib-preview-stage">
              {previewTheme.type === "fullscreen" ? (
                <ThemePreviewSurface
                  ...
                  videoSrc={previewTheme.settings?.backgroundVideo}
                  posterSrc={previewTheme.settings?.backgroundImage}
                  style={{
