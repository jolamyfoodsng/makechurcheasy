// ────────────────────────────────────────────────────────────────────────────
// Interactive Tutorial System – Service Planner
//
...
import "./ServicePlannerTutorial.css";
...
const STORAGE_KEY = "mce.service-planner.tutorial.completed";
...
interface ServicePlannerTutorialProps {
  isActive: boolean;
  onClose: () => void;
  onFinish: () => void;
}

export default function ServicePlannerTutorial({
  isActive,
  onClose,
  onFinish,
}: ServicePlannerTutorialProps) {
  ...
  const allSteps: TutorialStep[] = useMemo(() => [
    {
      target: "[data-spt-tutorial='welcome']",
      titleKey: "spt.step1.title",
      descKey: "spt.step1.desc",
      actionKey: "spt.step1.action",
      trigger: "none",
      panelPosition: "right",
    },
    {
      target: "[data-spt-tutorial='new-plan']",
      titleKey: "spt.step2.title",
      descKey: "spt.step2.desc",
      actionKey: "spt.step2.action",
      trigger: "none",
      panelPosition: "right",
    },
    {
      target: "[data-spt-tutorial='plans-list']",
      titleKey: "spt.step3.title",
      descKey: "spt.step3.desc",
      actionKey: "spt.step3.action",
      trigger: "none",
      panelPosition: "right",
    },
    {
      target: "[data-spt-tutorial='editor']",
      titleKey: "spt.step4.title",
      descKey: "spt.step4.desc",
      actionKey: "spt.step4.action",
      trigger: "none",
      panelPosition: "right",
    },
    {
      target: "[data-spt-tutorial='cues']",
      titleKey: "spt.step5.title",
      descKey: "spt.step5.desc",
      actionKey: "spt.step5.action",
      trigger: "none",
      panelPosition: "right",
    },
    {
      target: "[data-spt-tutorial='add-cue']",
      titleKey: "spt.step6.title",
      descKey: "spt.step6.desc",
      actionKey: "spt.step6.action",
      trigger: "none",
      panelPosition: "right",
    },
  ], []);
  ...
  if (isFinalStep) {
    return (
      <div className="spt-overlay" style={{ pointerEvents: "auto" }} onClick={onClose}>
        <div className="spt-panel spt-panel--final" style={{ top: panelRect.top, left: panelRect.left }}>
          <button className="spt-close" onClick={onClose} title="Close">
            <X size={16} />
          </button>

          <div className="spt-final-header">
            <div className="spt-final-icon">
              <Sparkles size={24} />
            </div>
            <h2 className="spt-final-title">{t("spt.final.title")}</h2>
            <p className="spt-final-desc">{t("spt.final.desc")}</p>
          </div>

          <div className="spt-final-checklist">
            {[
              "spt.final.check1",
              "spt.final.check2",
              "spt.final.check3",
              ...
              <div key={key} className="spt-final-check-item">
                <CheckCircle2 size={16} className="spt-check-icon" />
                <span>{t(key)}</span>
              </div>
            ))}
          </div>

          <div className="spt-final-actions">
            <button className="spt-btn spt-btn-primary" onClick={onFinish} title="Finish tutorial">
              {t("spt.final.finish")}
            </button>
          </div>
        </div>
  ...
  return (
    <div className="spt-overlay">
      ...
      >
        ...
        <div className="spt-panel-header">
          <div className="spt-step-badge">
        ...
        <div className="spt-panel-body">
          <div className="spt-step-icon-wrap">
            <Lightbulb size={18} />
          </div>
          <h3 className="spt-step-title">{t(currentStep.titleKey)}</h3>
          <p className="spt-step-desc">{t(currentStep.descKey)}</p>

          <div className="spt-action-hint">
            <ArrowRight size={14} />
            <span>{t(currentStep.actionKey)}</span>
          </div>

          {stepCompleted && (
            <div className="spt-step-complete">
              <CheckCircle2 size={16} />
              <span>{t("spt.common.done")}</span>
            </div>
          )}
        </div>

        <div className="spt-panel-footer">
          <button
            className="spt-btn spt-btn-ghost"
            onClick={handleSkip}
            title="Skip tutorial">
            <SkipForward size={14} /> {t("spt.common.skip")}
          </button>

          <div className="spt-nav-buttons">
            <button
              className="spt-btn"
              onClick={goPrev}
              title="Previous"
            ...
            <button
              className={`spt-btn spt-btn-primary ${!stepCompleted && currentStep.trigger !== "none" ? "spt-btn--waiting" : ""}`}
              disabled={!stepCompleted && currentStep.trigger !== "none"}
              onClick={goNext}
              title="Next step">
              {t("spt.common.next")} <ChevronRight size={14} />
            </button>
          </div>
        </div>
      </div>
...
export function isServicePlannerTutorialCompleted(): boolean {
  return localStorage.getItem(STORAGE_KEY) === "true";
}

export function markServicePlannerTutorialCompleted(): void {
  localStorage.setItem(STORAGE_KEY, "true");
}

export function resetServicePlannerTutorial(): void {
