// ────────────────────────────────────────────────────────────────────────────
// Interactive Tutorial System – Resources Page
//
// A guided onboarding assistant that highlights UI elements by elevating
// them above the dark backdrop (z-index manipulation) and walks the user
// through the Bible, Worship, and Media resource tabs.
// ────────────────────────────────────────────────────────────────────────────
...
interface ResourcesTutorialProps {
  isActive: boolean;
  onClose: () => void;
  onFinish: () => void;
}

export default function ResourcesTutorial({
  isActive,
  onClose,
  onFinish,
}: ResourcesTutorialProps) {
  ...
  const allSteps: TutorialStep[] = useMemo(() => [
    {
      target: "[data-res-tutorial='welcome']",
      titleKey: "rt.step1.title",
      descKey: "rt.step1.desc",
      actionKey: "rt.step1.action",
      trigger: "none",
      panelPosition: "right",
    },
    {
      target: "[data-res-tutorial='tabs']",
      titleKey: "rt.step2.title",
      descKey: "rt.step2.desc",
      actionKey: "rt.step2.action",
      trigger: "none",
      panelPosition: "right",
    },
    {
      target: "[data-res-tutorial='content']",
      titleKey: "rt.step3.title",
  ...
  if (isFinalStep) {
    return (
      <div className="rt-overlay" style={{ pointerEvents: "auto" }} onClick={onClose}>
        <div className="rt-panel rt-panel--final" style={{ top: panelRect.top, left: panelRect.left }}>
          <button className="rt-close" onClick={onClose} title="Close">
            <X size={16} />
          </button>

          <div className="rt-final-header">
            <div className="rt-final-icon">
              <Sparkles size={24} />
            </div>
            <h2 className="rt-final-title">{t("rt.final.title")}</h2>
            <p className="rt-final-desc">{t("rt.final.desc")}</p>
          </div>

          <div className="rt-final-checklist">
            {[
              "rt.final.check1",
              "rt.final.check2",
              "rt.final.check3",
              ...
              <div key={key} className="rt-final-check-item">
                <CheckCircle2 size={16} className="rt-check-icon" />
                <span>{t(key)}</span>
              </div>
            ))}
          </div>
          ...
          <div className="rt-final-actions">
            <button className="rt-btn rt-btn-primary" onClick={onFinish} title="Finish tutorial">
              {t("rt.final.finish")}
            </button>
          </div>
        </div>
    ...
    <div className="rt-overlay">
      ...
      >
        ...
        <div className="rt-panel-body">
          <div className="rt-step-icon-wrap">
            <Lightbulb size={18} />
          </div>
          <h3 className="rt-step-title">{t(currentStep.titleKey)}</h3>
          <p className="rt-step-desc">{t(currentStep.descKey)}</p>

          <div className="rt-action-hint">
            <ArrowRight size={14} />
            <span>{t(currentStep.actionKey)}</span>
          </div>

          {stepCompleted && (
            <div className="rt-step-complete">
              <CheckCircle2 size={16} />
              <span>{t("rt.common.done")}</span>
            </div>
          )}
        </div>

        <div className="rt-panel-footer">
          <button
            className="rt-btn rt-btn-ghost"
            onClick={handleSkip}
            title="Skip tutorial">
            <SkipForward size={14} /> {t("rt.common.skip")}
          </button>

          <div className="rt-nav-buttons">
            <button
              className="rt-btn"
              onClick={goPrev}
              title="Previous"
            ...
            <button
              className={`rt-btn rt-btn-primary ${!stepCompleted && currentStep.trigger !== "none" ? "rt-btn--waiting" : ""}`}
              disabled={!stepCompleted && currentStep.trigger !== "none"}
              onClick={goNext}
              title="Next step">
              {t("rt.common.next")} <ChevronRight size={14} />
            </button>
          </div>
        </div>
      </div>
...
export function isResourcesTutorialCompleted(): boolean {
  return localStorage.getItem(STORAGE_KEY) === "true";
}

export function markResourcesTutorialCompleted(): void {
  localStorage.setItem(STORAGE_KEY, "true");
}

export function resetResourcesTutorial(): void {
