...
import { nextAutoSongTitle, isDummyTitle } from "./songTitleAutoGen";
import { extractFirstLineAsTitle } from "./songTitleFromLyrics";
...
export default function WorshipSongEditor({ song, onClose, onSave }: WorshipSongEditorProps) {
  ...
  const titleWasAutoGeneratedRef = useRef(!song || isDummyTitle(song?.metadata.title));
  ...
  const handleLyricsChange = useCallback((nextLyrics: string) => {
    authoredLyricsRef.current = nextLyrics;
    setLyrics(nextLyrics);
    if (!titleWasAutoGeneratedRef.current && !isDummyTitle(title) && title.trim()) return;

    const derivedTitle = extractFirstLineAsTitle(nextLyrics);
    if (derivedTitle) {
      setTitle(derivedTitle);
    } else if (!nextLyrics.trim()) {
      setTitle(song?.metadata.title ?? nextAutoSongTitle());
    }
            ...
            <div className="ws-left-fields">
              <div>
                ...
                <input
                  ...
                  value={title}
                  onChange={(e) => {
                    titleWasAutoGeneratedRef.current = isDummyTitle(e.target.value);
