...
const onlineLyricsSearchCache = new Map<string, { timestamp: number; results: OnlineLyricsSearchResult[] }>();
const onlineLyricsSearchInflight = new Map<string, Promise<OnlineLyricsSearchResult[]>>();
...
export async function searchOnlineSongLyrics(query: string): Promise<OnlineLyricsSearchResult[]> {
  ...
  const cacheKey = cleanInlineText(resolvedQuery).toLowerCase();
  const now = Date.now();
  const cached = onlineLyricsSearchCache.get(cacheKey);
  if (cached && now - cached.timestamp < ONLINE_LYRICS_CACHE_TTL_MS) {
    return cached.results;
  }

  const inflight = onlineLyricsSearchInflight.get(cacheKey);
  if (inflight) {
    return inflight;
  }
  ...
  const request = (async () => {
    ...
    if (hasTauriRuntime()) {
      try {
        ...
        const cleaned = Array.isArray(results) ? results.map(cleanOnlineLyricsResult) : [];
        onlineLyricsSearchCache.set(cacheKey, { timestamp: Date.now(), results: cleaned });
        return cleaned;
      } catch (error) {
        invokeError = error;
    ...
    try {
      const cleaned = (await searchOnlineSongLyricsFallback(resolvedQuery)).map(cleanOnlineLyricsResult);
      onlineLyricsSearchCache.set(cacheKey, { timestamp: Date.now(), results: cleaned });
      return cleaned;
    } catch (fallbackError) {
      if (invokeError) {
  ...
  onlineLyricsSearchInflight.set(cacheKey, request);
  try {
    return await request;
  } finally {
    onlineLyricsSearchInflight.delete(cacheKey);
  }
}
