    ...
    try {
      ...
      if (existing) {
        ...
        if (this._isLive && this._liveText) {
          ...
          const packet = { slide, theme: this._liveTheme, live: true, blanked: this._isBlanked, timestamp: Date.now() };
  ...
  async pushSlide(
    ...
    try {
      ...
      if (theme?.backgroundImage && theme.backgroundImage.startsWith("data:")) {
        // Don't send massive base64 to the overlay — the image_source handles it
