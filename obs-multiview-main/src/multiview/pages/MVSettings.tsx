 ...
 *
 * Tabs:
 *   Layout   — OBS Connection, Canvas, Editor, Auto-Save, Appearance, Shortcuts, About, Danger Zone
 *   Branding — Church profile, logo, social handles, brand color, service defaults
 *   Bible    — Colour mode, Translation, Theme, Slide Config, Behaviour, Accessibility, Shortcuts
 */
...
type SettingsTab = "layout" | "branding" | "bible";
...
export function MVSettings() {
  ...
  const [bShortcutsOpen, setBShortcutsOpen] = useState(false);
    ...
    <div className="mv-page mv-settings">
      ...
      <div className="mv-settings-tabs" role="tablist">
        <button
          ...
          className={`mv-settings-tab${activeTab === "layout" ? " mv-settings-tab--active" : ""}`}
        ...
        <section className="mv-settings-section">
          ...
          {shortcutsOpen && (
            <div className="mv-shortcuts-grid">
              {(Array.from(grouped.entries()) as [ShortcutCategory, typeof SHORTCUTS[number][]][]).map(
                ([cat, items]) => (
                  <div key={cat} className="mv-shortcuts-group">
                    <h3 className="mv-shortcuts-cat">{CATEGORY_LABELS[cat]}</h3>
                    {items.map((s) => (
                      <div key={s.id} className="mv-shortcuts-item">
          ...
          <div className="mv-settings-about">
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
              ...
              <div>
                ...
                <p style={{ margin: 0, fontSize: 12, color: "var(--text-muted)" }}>Version 1.0.0</p>
              </div>
            </div>
            ...
            <div className="mv-settings-row" style={{ marginTop: 12 }}>
              <a
                className="mv-btn mv-btn--outline mv-btn--sm"
                href="https://github.com/Tayoakosile/obs-multiview"
                target="_blank"
      ...
      <div className="mv-settings-sections">
        ...
        <section className="mv-settings-section">
          ...
          <div className="mv-settings-field" style={{ maxWidth: 340 }}>
            <span className="mv-settings-field-label">Colour Mode</span>
            <div className="mv-theme-toggle-group" role="radiogroup" aria-label="Bible colour mode">
              {(["dark", "light", "system"] as const).map((mode) => (
                <button
                  key={mode}
                  role="radio"
                  aria-checked={bColorMode === mode}
                  className={`mv-theme-toggle-btn${bColorMode === mode ? " mv-theme-toggle-btn--active" : ""}`}
                  onClick={() => setBColorMode(mode)}
                >
                  <Icon name={mode === "dark" ? "dark_mode" : mode === "light" ? "light_mode" : "settings_brightness"} size={16} />
        ...
        <section className="mv-settings-section" style={{ borderBottom: "none" }}>
          <h2
            className="mv-settings-heading mv-settings-heading--clickable"
            onClick={() => setBShortcutsOpen((o) => !o)}
          >
            <Icon name="keyboard" size={18} style={{ verticalAlign: "text-bottom" }} />
            {" "}Keyboard Shortcuts
            <Icon name="expand_more" size={18} className="mv-settings-chevron" style={{ marginLeft: "auto", transition: "transform .2s", transform: bShortcutsOpen ? "rotate(180deg)" : undefined }} />
          </h2>
          <p className="mv-settings-desc">
            Bible interface keyboard shortcuts reference.
            {!bShortcutsOpen && (
              <>{" "}<button className="mv-link" onClick={() => setBShortcutsOpen(true)}>Show all</button></>
            )}
          </p>
