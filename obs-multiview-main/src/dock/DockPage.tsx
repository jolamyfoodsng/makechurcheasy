 ...
 * (dockBridge/dockClient) to send overlay commands.
 *
...
export default function DockPage() {
          ...
          setStaged((current) => {
            ...
            if (recovered.bible) {
              ...
              return {
                ...
                subtitle: recovered.bible.text?.substring(0, 60) || "",
                data: {
                  book: "",
                  chapter: 0,
                  verse: 0,
  ...
  const handleSendPreview = useCallback(async () => {
    ...
    try {
      ...
      // Also send via BroadcastChannel (if main app is in same process)
      const cmdType = `${staged.type}:send-preview`;
      dockClient.sendCommand({
        type: cmdType as DockCommandType,
        payload: staged.data,
        timestamp: Date.now(),
      });
    } catch (err) {
  ...
  const handleGoLive = useCallback(async () => {
    ...
    try {
      ...
      const cmdType = `${staged.type}:go-live`;
      dockClient.sendCommand({
        type: cmdType as DockCommandType,
        payload: staged.data,
        timestamp: Date.now(),
  ...
  const handleClear = useCallback(async () => {
    ...
    try {
      ...
      // Also send via BroadcastChannel
      const cmdType = `${staged.type}:clear`;
      dockClient.sendCommand({
        type: cmdType as DockCommandType,
        payload: null,
        timestamp: Date.now(),
    ...
    <div className="dock-root">
        ...
        <div className="dock-status-bar__left">
          <div
            className={`dock-status-dot ${
              obsConnected ? "dock-status-dot--connected" : "dock-status-dot--disconnected"
            }`}
          />
      ...
      {staged && activeTab !== "media" && (
        <div className="dock-staged">
          ...
          {staged.subtitle && <div className="dock-staged__sub">{staged.subtitle}</div>}
        </div>
      )}
