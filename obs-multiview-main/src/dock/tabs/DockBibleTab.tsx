...
const TRANSLATIONS = ["KJV", "NIV", "ESV", "NLT", "NASB", "NKJV"];
...
export default function DockBibleTab({ staged: _staged, onStage }: Props) {
  ...
  const [selectedLTTheme, setSelectedLTTheme] = useState<DockLTThemeRef | null>(null);
    ...
    (async () => {
      ...
      onStage({
        ...
        data: {
          ...
          verseText: text,
          overlayMode,
          theme: overlayMode === "fullscreen" ? selectedBibleTheme.id : (selectedLTTheme?.id ?? selectedBibleTheme.id),
          bibleThemeSettings: overlayMode === "fullscreen" ? selectedBibleTheme.settings : undefined,
          ltTheme: overlayMode === "lower-third" ? selectedLTTheme : undefined,
        },
      });
    })();
    return () => { cancelled = true; };
  }, [translation, selectedBook, selectedChapter, selectedVerse, overlayMode, selectedBibleTheme, selectedLTTheme, fetchVerseText, onStage]);
  ...
  }, [overlayMode, selectedBook, selectedChapter, selectedVerse, _verseText, translation, selectedBibleTheme, selectedLTTheme, onStage]);
  ...
  const handlePickResult = useCallback(
    ...
    [translation, selectedBibleTheme, selectedLTTheme, overlayMode, fetchVerseText, onStage]
  );
  ...
  const handleSearchKeyDown = useCallback(
    (e: React.KeyboardEvent) => {
      ...
      if (e.key === "ArrowDown") {
        ...
        e.preventDefault();
        handlePickResult(searchResults[activeIdx]);
  ...
  const handleSelectVerse = useCallback(
    ...
    [selectedBook, selectedChapter, translation, selectedBibleTheme, selectedLTTheme, overlayMode, fetchVerseText, onStage]
  );
  ...
  const handleDoubleClickVerse = useCallback(
    async (v: number) => {
      ...
      const stageData = {
        ...
        overlayMode,
        theme: overlayMode === "fullscreen" ? selectedBibleTheme.id : (selectedLTTheme?.id ?? selectedBibleTheme.id),
        bibleThemeSettings: overlayMode === "fullscreen" ? (selectedBibleTheme.settings as unknown as Record<string, unknown>) : undefined,
        ltTheme: overlayMode === "lower-third" ? selectedLTTheme ?? undefined : undefined,
      };
  ...
  return (
    <>
      ...
      {overlayMode === "fullscreen" ? (
        ...
        <DockLTThemePicker
          selectedThemeId={selectedLTTheme?.id ?? null}
          onSelect={(t) => setSelectedLTTheme(t)}
          category="bible"
          label="Bible Lower Third Theme"
          tags={["bible", "scripture", "verse", "reference", "word"]}
        />
      )}
    </>
  );
