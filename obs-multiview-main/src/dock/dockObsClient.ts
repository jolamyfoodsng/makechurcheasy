...
class DockObsClient {
  ...
  private async ensureOverlaySource(
    ...
    // 3. Move to top of z-order (highest index = rendered last = on top)
    try {
      const updated = await this.call("GetSceneItemList", { sceneName }) as {
        sceneItems: Array<{ sourceName: string; sceneItemId: number; sceneItemIndex: number }>;
      };
      const topIndex = updated.sceneItems.length - 1;
  ...
  private stripThemeDataUris(
    ...
    if (bgImage && bgImage.startsWith("data:")) {
      // Background is handled by a separate OBS image_source behind the
      // browser source (via ensureFullscreenBg), so just mark as sentinel.
      clean.backgroundImage = "__BG_SOURCE__";
    }
  ...
  private async ensureFullscreenBg(
    ...
    if (bgImage === "__BG_SOURCE__" || bgImage === "__FROM_CSS__" || bgImage === "__FROM_LOCALSTORAGE__") return;

    const hasImage = bgImage && !bgImage.startsWith("data:") && bgImage.startsWith("http");

    if (hasImage) {
      // Use an image_source
      await this._ensureBgImageSource(sceneName, bgImage, enable);
    } else {
      // Use a color_source_v3 with the theme's backgroundColor
      await this._ensureBgColorSource(sceneName, bgColor, enable);
    }
  }
   ...
   */
  private async _ensureBgImageSource(sceneName: string, imageUrl: string, enable: boolean): Promise<void> {
    const sourceName = DOCK_FS_BG_SOURCE;
    ...
    await this._positionBgBelowOverlays(sceneName, sceneItem.sceneItemId);
   ...
   */
  private async _ensureBgColorSource(sceneName: string, bgColor: string, enable: boolean): Promise<void> {
    const sourceName = DOCK_FS_BG_SOURCE;
  ...
  /**
   * Position the background source just below the lowest overlay source,
   * so it sits behind all overlays but above other scene content.
   */
  private async _positionBgBelowOverlays(sceneName: string, bgSceneItemId: number): Promise<void> {
    try {
      const resp = await this.call("GetSceneItemList", { sceneName }) as {
        sceneItems: Array<{ sourceName: string; sceneItemId: number; sceneItemIndex: number }>;
      };

      const overlayNames = new Set([DOCK_BIBLE_SOURCE, DOCK_WORSHIP_SOURCE, DOCK_LT_SOURCE, DOCK_TICKER_SOURCE]);
      const overlayItems = resp.sceneItems.filter((i) => overlayNames.has(i.sourceName));

      if (overlayItems.length === 0) return;

      // Find the lowest overlay index
      const lowestOverlayIndex = Math.min(...overlayItems.map((i) => i.sceneItemIndex));
      const targetIndex = Math.max(0, lowestOverlayIndex);
   ...
   */
  private async hideFullscreenBg(sceneName: string): Promise<void> {
    await this.hideOverlaySource(sceneName, DOCK_FS_BG_SOURCE);
  }
   ...
   */
  async clearAllOverlays(keepSources: string | string[] | null = null, sceneName?: string): Promise<void> {
  ...
  async pushBible(data: {
    ...
    const { sceneName, studioMode } = await this.getTargetScene(live);
    ...
    const prevMode = this._lastOverlayMode[DOCK_BIBLE_SOURCE];
    const modeChanged = prevMode !== undefined && prevMode !== mode;
    this._lastOverlayMode[DOCK_BIBLE_SOURCE] = mode;

    let url: string;
    let themeCss = "";
    if (mode === "lower-third") {
      // ── Lower-third: direct browser source in user's scene ──
      // Clear all OTHER overlays first
      await this.clearAllOverlays(DOCK_BIBLE_SOURCE, sceneName);

      await this.ensureOverlaySource(sceneName, DOCK_BIBLE_SOURCE, 1920, 1080, shouldEnable);
      ...
      await this.hideFullscreenBg(sceneName);
      await this.hideSceneSource(sceneName, DOCK_BIBLE_SCENE);
    } else {
      // ── Fullscreen: dedicated scene approach ──
      // Clear all OTHER overlays — keep the Bible scene, its overlay source, and its BG
      await this.clearAllOverlays([DOCK_BIBLE_SCENE, DOCK_BIBLE_SOURCE, DOCK_FS_BG_SOURCE], sceneName);

      // 1. Ensure the dedicated Bible scene exists
      await this.ensureDedicatedScene(DOCK_BIBLE_SCENE);

      // 2. Inside the dedicated scene, ensure BG + overlay sources exist
      await this.ensureOverlaySource(DOCK_BIBLE_SCENE, DOCK_BIBLE_SOURCE, 1920, 1080, true);
      await this.ensureFullscreenBg(DOCK_BIBLE_SCENE, data.bibleThemeSettings as Record<string, unknown> | null, true);

      // 3. Add the dedicated scene as a nested scene-source in the user's target scene
      await this.ensureSceneSourceInTarget(sceneName, DOCK_BIBLE_SCENE, shouldEnable);
      ...
      url = this.buildBibleUrl(slide, live, false, cleanSettings, mode);
    }
    ...
    if (mode === "lower-third") {
      await this.hideInOppositeScene(live, [DOCK_BIBLE_SOURCE, DOCK_FS_BG_SOURCE]);
    } else {
      await this.hideInOppositeScene(live, [DOCK_BIBLE_SOURCE], [DOCK_BIBLE_SCENE]);
    }

    await this.setBrowserSourceUrl(DOCK_BIBLE_SOURCE, url, modeChanged, themeCss || undefined);
  ...
  async clearBible(): Promise<void> {
    const lastMode = this._lastOverlayMode[DOCK_BIBLE_SOURCE] ?? "fullscreen";
    let url: string;
    if (lastMode === "lower-third") {
      url = this.buildBibleLowerThirdUrl("", "", false, true);
    } else {
      url = this.buildBibleUrl(null, false, true, null);
    }
    try { await this.setBrowserSourceUrl(DOCK_BIBLE_SOURCE, url); } catch { /* ignore */ }

    // Wait for exit animation before hiding the source
    await new Promise((r) => setTimeout(r, 800));

    // Hide in current scenes (both direct sources and dedicated scene sources)
    try {
      const { sceneName: progScene } = await this.getTargetScene(true);
      if (progScene) {
        await this.hideOverlaySource(progScene, DOCK_BIBLE_SOURCE);
        await this.hideFullscreenBg(progScene);
        await this.hideSceneSource(progScene, DOCK_BIBLE_SCENE);
      }
    } catch { /* ignore */ }
    try {
      const { sceneName: prevScene } = await this.getTargetScene(false);
      if (prevScene) {
        await this.hideOverlaySource(prevScene, DOCK_BIBLE_SOURCE);
        await this.hideFullscreenBg(prevScene);
        await this.hideSceneSource(prevScene, DOCK_BIBLE_SCENE);
      }
    } catch { /* ignore */ }
  ...
  async pushLowerThird(data: {
    ...
    await this.clearAllOverlays(DOCK_LT_SOURCE, sceneName);

    // Ensure overlay source exists in target scene (auto-creates if needed)
    await this.ensureOverlaySource(sceneName, DOCK_LT_SOURCE, 1920, 1080, shouldEnable);
    ...
    await this.hideInOppositeScene(live, [DOCK_LT_SOURCE]);

    await this.setBrowserSourceUrl(DOCK_LT_SOURCE, url);
  ...
  async clearLowerThirds(): Promise<void> {
    ...
    try { await this.setBrowserSourceUrl(DOCK_LT_SOURCE, url); } catch { /* ignore */ }

    // Wait for exit animation before hiding the source
    await new Promise((r) => setTimeout(r, 800));

    // Hide in current scenes
    try {
      const { sceneName: progScene } = await this.getTargetScene(true);
      if (progScene) await this.hideOverlaySource(progScene, DOCK_LT_SOURCE);
    } catch { /* ignore */ }
    try {
      const { sceneName: prevScene } = await this.getTargetScene(false);
      if (prevScene) await this.hideOverlaySource(prevScene, DOCK_LT_SOURCE);
    } catch { /* ignore */ }
  ...
  private buildWorshipFullscreenUrl(
    ...
    const slide = sectionText ? {
      ...
      reference: `${songTitle}${artist ? ` — ${artist}` : ""} · ${sectionLabel}`,
      text: sectionText,
  ...
  private buildWorshipLyricsUrl(
    ...
    const songInfo = [artist, songTitle].filter(Boolean).join(" – ");
    ...
    const values: Record<string, string> = {
      ...
      songName: songTitle || line1,
      artist: artist || "",
      songInfo: songInfo || songTitle || line1,
      title: songTitle || line1,
      subtitle: artist || line2 || "Worship",
      text: sectionText || line1,
      headline: songTitle || line1,
      details: line2 || artist || "Worship Service",
      quote: sectionText || line1,
      reference: artist || "Worship",
      song: songTitle || line1,
      meta: songInfo || "Worship",
      label: sectionLabel || "Now Playing",
    };
  ...
  async pushWorshipLyrics(data: {
    ...
    const prevMode = this._lastOverlayMode[DOCK_WORSHIP_SOURCE];
    const modeChanged = prevMode !== undefined && prevMode !== mode;
    this._lastOverlayMode[DOCK_WORSHIP_SOURCE] = mode;
    ...
    if (mode === "fullscreen") {
      ...
      await this.clearAllOverlays([DOCK_WORSHIP_SCENE, DOCK_WORSHIP_SOURCE, DOCK_FS_BG_SOURCE], sceneName);

      // 1. Ensure the dedicated Worship scene exists
      await this.ensureDedicatedScene(DOCK_WORSHIP_SCENE);

      // 2. Inside the dedicated scene, ensure BG + overlay sources exist
      await this.ensureOverlaySource(DOCK_WORSHIP_SCENE, DOCK_WORSHIP_SOURCE, 1920, 1080, true);
      await this.ensureFullscreenBg(DOCK_WORSHIP_SCENE, data.bibleThemeSettings as Record<string, unknown> | null, true);

      // 3. Add the dedicated scene as a nested scene-source in the user's target scene
      await this.ensureSceneSourceInTarget(sceneName, DOCK_WORSHIP_SCENE, shouldEnable);
      ...
      // ── Lower-third: direct browser source in user's scene ──
      await this.clearAllOverlays(DOCK_WORSHIP_SOURCE, sceneName);

      await this.ensureOverlaySource(sceneName, DOCK_WORSHIP_SOURCE, 1920, 1080, shouldEnable);
      ...
      await this.hideFullscreenBg(sceneName);
      await this.hideSceneSource(sceneName, DOCK_WORSHIP_SCENE);
    }

    // Hide in opposite scene BEFORE setting URL — global URL change must not leak
    if (mode === "fullscreen") {
      await this.hideInOppositeScene(live, [DOCK_WORSHIP_SOURCE], [DOCK_WORSHIP_SCENE]);
    } else {
      await this.hideInOppositeScene(live, [DOCK_WORSHIP_SOURCE]);
    }

    await this.setBrowserSourceUrl(DOCK_WORSHIP_SOURCE, url, modeChanged, themeCss || undefined);
  ...
  async clearWorshipLyrics(): Promise<void> {
    const lastMode = this._lastOverlayMode[DOCK_WORSHIP_SOURCE] ?? "lower-third";
    const url = lastMode === "fullscreen"
      ? this.buildWorshipFullscreenUrl("", "", "", "", false, true)
      : this.buildWorshipLyricsUrl("", "", "", "", false, true);
    try { await this.setBrowserSourceUrl(DOCK_WORSHIP_SOURCE, url); } catch { /* ignore */ }
    ...
    try {
      const { sceneName: progScene } = await this.getTargetScene(true);
      if (progScene) {
        await this.hideOverlaySource(progScene, DOCK_WORSHIP_SOURCE);
        await this.hideFullscreenBg(progScene);
        await this.hideSceneSource(progScene, DOCK_WORSHIP_SCENE);
      }
    } catch { /* ignore */ }
    try {
      const { sceneName: prevScene } = await this.getTargetScene(false);
      if (prevScene) {
        await this.hideOverlaySource(prevScene, DOCK_WORSHIP_SOURCE);
        await this.hideFullscreenBg(prevScene);
        await this.hideSceneSource(prevScene, DOCK_WORSHIP_SCENE);
      }
    } catch { /* ignore */ }
  ...
  async pushTicker(data: {
    ...
    // Clear all OTHER overlays first so previous overlay doesn't persist
    await this.clearAllOverlays(DOCK_TICKER_SOURCE, sceneName);

    await this.ensureOverlaySource(sceneName, DOCK_TICKER_SOURCE, 1920, 1080, shouldEnable);
    ...
    // Hide in opposite scene BEFORE setting URL — global URL change must not leak
    await this.hideInOppositeScene(live, [DOCK_TICKER_SOURCE]);

    await this.setBrowserSourceUrl(DOCK_TICKER_SOURCE, url);
  ...
  async pushMedia(filePath: string, fileName: string, live: boolean): Promise<void> {
    ...
    if (isImage) {
      ...
      await this._hideMediaSource(sceneName, DOCK_MEDIA_VIDEO_SOURCE);

      await this._ensureNativeMediaSource(
        sceneName, DOCK_MEDIA_IMAGE_SOURCE, "image_source",
        { file: filePath },
        shouldEnable,
      );
    } else {
      ...
      await this._hideMediaSource(sceneName, DOCK_MEDIA_IMAGE_SOURCE);

      await this._ensureNativeMediaSource(
        sceneName, DOCK_MEDIA_VIDEO_SOURCE, "ffmpeg_source",
        {
          local_file: filePath,
          looping: true,
          is_local_file: true,
      ...
      try {
        await this.call("TriggerMediaInputAction", {
          inputName: DOCK_MEDIA_VIDEO_SOURCE,
  ...
  async clearTicker(): Promise<void> {
    ...
    try {
      const { sceneName: progScene } = await this.getTargetScene(true);
      if (progScene) await this.hideOverlaySource(progScene, DOCK_TICKER_SOURCE);
    } catch { /* ignore */ }
    try {
      const { sceneName: prevScene } = await this.getTargetScene(false);
      if (prevScene) await this.hideOverlaySource(prevScene, DOCK_TICKER_SOURCE);
    } catch { /* ignore */ }
