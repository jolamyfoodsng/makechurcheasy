...
import type { LTSize, LTFontSize, LTPosition, LTAnimationIn } from "../../lowerthirds/types";
...
export function LowerThirdsModule({ isActive = true }: LowerThirdsModuleProps) {
  ...
  const handleRestoreVersion = useCallback(
    (snapshot: LTVersionSnapshot) => {
      const restoredTheme = LT_ALL_THEMES.find((t) => t.id === snapshot.themeId);
      if (!restoredTheme) {
  ...
  const handlePickCategory = useCallback((catId: LTPresetCategoryId) => {
    ...
    const chosenTheme = themes.find((t) => t.id === defaultThemeId) ?? themes[0];
  ...
  const handleThemeSelectFromDropdown = useCallback((nextThemeId: string) => {
    const nextTheme = LT_ALL_THEMES.find((th) => th.id === nextThemeId);
    if (!nextTheme) return;
        ...
        <aside className="lt-page-sidebar-left" style={{ position: "relative" }}>
          ...
          <div className="lt-page-theme-list">
            {ltPresets.map((p) => {
              ...
              const matchedTheme = LT_ALL_THEMES.find((t) => t.id === p.themeId);
