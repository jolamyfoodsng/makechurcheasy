...
import { getAllSongs, saveSong, deleteSong, syncSongsToDock } from "../../worship/worshipDb";
...
export function WorshipModule({
  ...
  const pushLiveSlideWithTheme = useCallback(async (theme: BibleThemeSettings) => {
    ...
    await worshipObsService.pushSlide(
      slide.content,
      `${selectedSong?.metadata.title ?? ""} — ${slide.label}`,
      theme,
      true,
      false
    );
  ...
  const buildWorshipLowerThirdValues = useCallback((theme: LowerThirdTheme): Record<string, string> => {
    ...
    const songName = selectedSong?.metadata.title?.trim() ?? "";
    const artist = selectedSong?.metadata.artist?.trim() ?? "";
    const songInfo = [artist, songName].filter(Boolean).join(" - ");
    const quote = slideText || line1;
    const subtitle = artist || line2 || "Worship Team";
    const meta = songInfo || "Worship";
    const details = line2 || artist || "Worship Service";
    const reference = artist || "Worship";
    ...
    setIfPresent("songName", songName || line1);
    setIfPresent("artist", artist);
    setIfPresent("songInfo", songInfo || songName || line1);
    setIfPresent("title", songName || line1);
    setIfPresent("subtitle", subtitle);
    setIfPresent("text", slideText || line1);
    setIfPresent("verseText", slideText || line1);
    setIfPresent("headline", songName || line1);
    setIfPresent("details", details);
    setIfPresent("name", songName || line1);
    setIfPresent("quote", quote);
    setIfPresent("reference", reference);
    setIfPresent("song", songName || line1);
    setIfPresent("meta", meta);

    if ("label" in values && !values.label.trim()) {
      values.label = "Now Playing";
    }
  ...
  const handleSendWorshipFullToScene = useCallback(async (sceneName: string) => {
    ...
    const ref = slide ? `${selectedSong?.metadata.title ?? ""} — ${slide.label}` : "";
            ...
            <div className="worship-slides-grid">
              {importSlides.map((slide, index) => (
                <div key={slide.id} className={`worship-slide-card${slide.isContinuation ? " cont" : ""}`}>
                  ...
                  <div className="worship-slide-card-foot">Slide {index + 1}</div>
                </div>
              ))}
  ...
  return (
    <div className="worship-home">
      ...
      <div className="worship-header">
        <div className="worship-header-left">
          <button className="worship-nav-btn" onClick={() => navigate(homePath)} title="Back to Dashboard">
            <Icon name="arrow_back" size={20} />
            Layouts
          </button>
          <span className="worship-header-divider" />
          <span className="worship-header-title">
              ...
              <div className="worship-center-head">
                <div className="worship-center-head-info">
                  ...
                  <div>
                    ...
                    <span className="worship-center-head-artist">{selectedSong.metadata.artist}</span>
                  </div>
                </div>
          ...
          <div className="worship-live-strip-rail">
            {songSlides.map((slide, index) => {
              ...
              return (
                ...
                >
                  <span className="worship-live-strip-slide-label">{slide.label || `Slide ${index + 1}`}</span>
