...
import { getChapter, getChapterCount, getVerseCount } from "../../bible/bibleData";
...
export function BibleModule({
  ...
  // Utility strip: 'none' | 'favorites' | 'history'
  const [activeUtilityTab, setActiveUtilityTab] = useState<"none" | "favorites" | "history">("none");
  ...
  const sendVerseToObs = useCallback(async (book: string, chapter: number, verse: number): Promise<boolean> => {
    ...
    const passage = await getChapter(book, chapter, state.translation);
  ...
  }, [isActive, handleNextVerse, handlePrevVerse, handleJumpVerseForward, handleJumpVerseBackward, handleNextChapter, handlePrevChapter, handleClear, handleToggleFavoriteCurrent, state.themes, setTheme]);
  ...
  const toggleUtilityTab = useCallback((tab: "favorites" | "history") => {
    setActiveUtilityTab(prev => prev === tab ? "none" : tab);
  }, []);
  ...
  const visibleFullThemes = useMemo(
    () => state.themes.filter((theme) => !theme.hidden),
    [state.themes],
  );
  ...
  const activeFullTheme = useMemo(() => {
    ...
    if (activeTheme && !activeTheme.hidden) return activeTheme;
            ...
            <div className="bible-modal-body">
              ...
              {layoutMode === "fullscreen" && (
                <div className="bible-theme-modal-grid">
                  {state.themes.filter(t => !t.hidden).map((theme, idx) => {
