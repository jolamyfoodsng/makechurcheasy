...
export function MVObsThemes() {
  ...
  const loadObsThemes = useCallback(async () => {
    try {
      ...
      console.warn("[MVObsThemes] Failed to load Broadcast themes:", error);
  ...
  const handleCreateObsTheme = async () => {
    ...
    try {
      ...
      setToastMessage("Created new Broadcast theme.");
    } catch (error) {
      console.error("[MVObsThemes] Failed to create Broadcast theme:", error);
      setToastMessage("Could not create Broadcast theme.");
    } finally {
  ...
  const handleSaveObsTheme = async () => {
    ...
    try {
      ...
      const themeToSave: BibleTheme = {
        ...
        description: obsThemeDescription.trim() || "Custom fullscreen Broadcast theme.",
        source: "custom",
        templateType: "fullscreen",
      ...
      console.error("[MVObsThemes] Failed to save Broadcast theme:", error);
      setToastMessage("Could not save Broadcast theme.");
    } finally {
      setObsThemeBusy(false);
    }
  };
  ...
  const handleDeleteObsTheme = async (themeToDelete: BibleTheme) => {
    ...
    try {
      ...
      console.error("[MVObsThemes] Failed to delete Broadcast theme:", error);
      setToastMessage("Could not delete Broadcast theme.");
    } finally {
      setObsThemeBusy(false);
      ...
      <main className="tpldash-main">
        <section className="tpldash-center">
          <header className="tpldash-header">
            <div>
              <h2 className="tpldash-title">Broadcast Output Themes</h2>
              <p className="tpldash-subtitle">Manage Bible and Worship fullscreen themes sent to Broadcast.</p>
            </div>
