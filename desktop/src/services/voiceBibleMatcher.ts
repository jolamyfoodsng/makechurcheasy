...
/** Normalize modern English phrases into Bible-friendly search text */
function normalizeBibleLanguage(text: string): string {
  let out = text.toLowerCase();
  // Multi-word replacements first (order matters)
  out = out.replace(/\bwill not\b/g, "not");
  out = out.replace(/\bdo not\b/g, "not");
  out = out.replace(/\bdon'?t\b/g, "not");
  out = out.replace(/\bwon'?t\b/g, "not");
  out = out.replace(/\byourself\b/g, "thyself");
  out = out.replace(/\byours\b/g, "thine");
  out = out.replace(/\byour\b/g, "thy");
  out = out.replace(/\byou\b/g, "thee");
  out = out.replace(/\bor\b/g, "nor");
  // Strip conversational noise
  out = out.replace(/\bas\s+(we|i|they|you)\s+saw\b/g, " ");
  out = out.replace(/\bas\s+(we|i|they|you)\s+(said|mentioned|talked|spoke|read|going)\b/g, " ");
  out = out.replace(/\b(as|we|i|they|you|it|he|she)\b/g, " ");
  out = out.replace(/\b(saw|said|mentioned|talked|spoke|read|looking|look|going|went|came|think|know)\b/g, " ");
...
function normalizeSearchText(value: string): string {
  return value
    ...
    .replace(/[^\w\s:]/g, " ")
    // If a book name is followed directly by "verse N" with no chapter,
    // insert chapter 1: "philippians verse 10" → "philippians 1:10"
    .replace(/\b([a-z]+)\s+verse\s+(\d+)\b/g, (match, book, verse) => {
      if (BOOK_CHAPTERS[book] !== undefined) {
        return `${book} 1:${verse}`;
      }
      return match;
    })
...
/** Simple Bible-English stemmer — strips common suffixes for fuzzy matching */
function bibleStem(word: string): string {
  if (word.length <= 3) return word;
  // "called" → "call", "forsaken" → "forsake", "loved" → "love"
  if (word.endsWith("ed") && word.length > 5) return word.slice(0, -2);
  if (word.endsWith("ing") && word.length > 6) return word.slice(0, -3);
  if (word.endsWith("eth") && word.length > 6) return word.slice(0, -3);
...
async function normalizeQuoteHeuristics(
  ...
  const normalized = normalizeSearchText(normalizeBibleLanguage(value));
...
function scoreLexicalMatch(
  ...
  for (const token of queryTokens) {
    const stem = bibleStem(token);
    if (entry.paddedText.includes(` ${token} `) || entry.paddedText.includes(` ${stem} `)) {
      tokenMatches += 1;
    } else {
      // Check if any word in the entry starts with the stem
      const words = entry.paddedText.split(/\s+/);
      for (const word of words) {
        if (word.startsWith(stem) && word.length >= stem.length && word.length <= stem.length + 3) {
          tokenMatches += 1;
          break;
        }
      }
...
function buildReferenceQuery(transcript: string): string {
  return normalizeReferenceSpeech(transcript)
    ...
    .replace(/\b(our|your|the|to|into|in)\b/g, " ")
    .replace(/\b(bible|scripture|passage|text)\b/g, " ")
    // Strip conversational noise that surrounds Bible references
    .replace(/\bas\s+(we|i|they|you)\s+(saw|said|mentioned|talked|spoke|read|going)\b/g, " ")
    .replace(/\bas\s+(we|i|they|you)\b/g, " ")
    .replace(/\b(as|we|i|they|you|it|he|she)\b/g, " ")
    .replace(/\b(saw|said|mentioned|talked|spoke|read|looking|look|going|went|came|think|know|see)\b/g, " ")
    // Compact chapter/verse patterns
    .replace(/\bchapter\s+(\d+)\s+verse\s+(\d+)\b/g, "$1:$2")
    .replace(/\bchapter\s+(\d+)\b/g, "$1")
    .replace(/\b(\d+)\s+verse\s+(\d+)\b/g, "$1:$2")
    .replace(/\b(\d+)\s+vs\s+(\d+)\b/g, "$1:$2")
    .replace(/\b(\d+)\s*[-–—]\s*(\d+)\b/g, "$1:$2")
    // "bookname verse N" without chapter → insert chapter 1
    .replace(/\b([a-z]+)\s+verse\s+(\d+)\b/g, (_match, book, verse) => {
      if (BOOK_CHAPTERS[book] !== undefined) {
        return `${book} 1:${verse}`;
      }
      return `:${verse}`;
...
async function runLexicalSearch(
  ...
  const normalizedQuery = normalizeSearchText(normalizeBibleLanguage(transcript));
  const queryTokens = tokenize(normalizeBibleLanguage(transcript));
...
export async function resolveVoiceBibleIntent(
  ...
  const plannedCommand = options?.fastInterim || options?.lexicalOnly || options?.quoteOnly
    ? null
  ...
  const normalizedCommand = options?.lexicalOnly || options?.quoteOnly
    ? null
    : options?.fastInterim
      ? settings.semanticMode === "local"
  ...
  const normalizedQuoteQuery = options?.lexicalOnly
    ? null
    : options?.fastInterim
      ? settings.semanticMode === "local"
  ...
  const semanticQuery =
    (normalizedQuoteQuery && normalizedQuoteQuery !== trimmed
      ...
      : heuristicQuoteQuery && heuristicQuoteQuery !== trimmed
        ? heuristicQuoteQuery
        : trimmed);
  const reranked = options?.lexicalOnly || options?.quoteOnly
    ? combined
    : options?.fastInterim
      ? combined
      : settings.semanticMode === "local"
