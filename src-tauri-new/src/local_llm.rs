...
const LOCAL_LLM_DOWNLOAD_TIMEOUT_SECS: u64 = 20 * 60;
const LOCAL_LLM_MODEL_URLS: &[&str] = &[
...
fn resolve_install_source(source_path: Option<String>) -> Result<PathBuf, String> {
    match source_path {
        ...
        _ => detect_install_source_path(None).ok_or_else(|| {
            format!(
                "Local AI helper was not found in the app data folder or Downloads. Expected {}.",
...
fn install_local_llm_from_download(target: &Path) -> Result<(), String> {
    let temp_target = target.with_extension("gguf.part");
    let client = Client::builder()
        .timeout(Duration::from_secs(LOCAL_LLM_DOWNLOAD_TIMEOUT_SECS))
        .build()
        .map_err(|e| format!("Failed to configure local AI download client: {}", e))?;
    let mut last_error: Option<String> = None;

    for url in LOCAL_LLM_MODEL_URLS {
        let _ = fs::remove_file(&temp_target);
        let download_result: Result<(), String> = (|| {
            let mut response = client
                .get(*url)
                .header(reqwest::header::USER_AGENT, "MakeChurchEasy/1.0")
                .send()
                .map_err(|e| format!("Failed to download local AI helper: {}", e))?
                .error_for_status()
                .map_err(|e| format!("Local AI helper download returned an error: {}", e))?;

            let mut file = fs::File::create(&temp_target)
                .map_err(|e| format!("Failed to create temporary local AI file: {}", e))?;
            let mut buffer = [0_u8; 65_536];

            loop {
                let bytes_read = response
                    .read(&mut buffer)
                    .map_err(|e| format!("Failed while downloading local AI helper: {}", e))?;
                if bytes_read == 0 {
                    break;
                }

                file.write_all(&buffer[..bytes_read])
                    .map_err(|e| format!("Failed while writing local AI helper: {}", e))?;
            }

            file.flush()
                .map_err(|e| format!("Failed to finalize local AI helper download: {}", e))?;

            if !is_valid_model_file(&temp_target) {
                return Err("Downloaded local AI helper is incomplete.".to_string());
            }
...
pub(crate) fn install_local_llm_model(
    ...
    let target = local_llm_model_path()?;
    let requested_source = source_path.as_deref().unwrap_or("").trim();

    if requested_source.is_empty() && is_valid_model_file(&target) {
        write_local_llm_install_marker();
        return current_local_llm_status();
    }

    match resolve_install_source(source_path) {
        Ok(source) => {
            if source != target {
                install_local_llm_from_path(&source, &target)?;
            }
        ...
        Err(_) => {
            install_local_llm_from_download(&target)?;
        }
    }
